...
import ModelDetections.BoundingBox;
...
public class YOLOClassifier {
     ...
     */
    public List<Detection> classifyImage(final float[] tensorFlowOutput, final List<String> labels) {
        ...
        PriorityQueue<Detection> priorityQueue = new PriorityQueue(MAX_RECOGNIZED_CLASSES, new RecognitionComparator());
    ...
    private void calculateTopPredictions(final BoundingBox boundingBox, final PriorityQueue<Detection> predictionQueue,
        ...
        for (int i=0; i<boundingBox.getClasses().length; i++) {
            ...
            if (confidenceInClass > THRESHOLD) {
                predictionQueue.add(new Detection(argMax.getIndex(), labels.get(argMax.getIndex()), (float) confidenceInClass,
                        new BoxPos((float) (boundingBox.getX() - boundingBox.getWidth() / 2),
    ...
    private List<Detection> getRecognition(final PriorityQueue<Detection> priorityQueue) {
        List<Detection> recognitions = new ArrayList();

        if (priorityQueue.size() > 0) {
            // Best recognition
        	Detection bestRecognition = priorityQueue.poll();
            recognitions.add(bestRecognition);

            for (int i = 0; i < Math.min(priorityQueue.size(), MAX_RESULTS); ++i) {
                Detection recognition = priorityQueue.poll();
                boolean overlaps = false;
                for (Detection previousRecognition : recognitions) {
    ...
    private float getIntersectionProportion(BoxPos primaryShape, BoxPos secondaryShape) {
    ...
    private boolean overlaps(BoxPos primary, BoxPos secondary) {
    ...
    private class RecognitionComparator implements Comparator<Detection> {
        @Override
        public int compare(final Detection recognition1, final Detection recognition2) {
